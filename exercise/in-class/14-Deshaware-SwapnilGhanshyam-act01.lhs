...
reverse : The reverse is a library function which returns the value in reverse order. 
It only takes list (polymorphic) as input and not tuple. 
For instance, 
...
We turned of the implicit use of the library function reverse by the following command, as every module implicitly imports prelude
    import Prelude hiding(reverse)
...
The transfer function is takes an argument as tuple of arity two, with both the components are of type list. It is declare as
    transfer :: ([a], [a]) -> ([a], [a])
